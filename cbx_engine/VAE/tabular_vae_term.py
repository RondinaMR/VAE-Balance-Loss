...
# create logger
module_logger = logging.getLogger('engine.tabular_vae_term')
...
class TabularVAETerm(VAEInterface, nn.Module):
    ...
    def setup(self):
        ...
        self.logger = logging.getLogger('engine.tabular_vae.TabularVAETerm')
    ...
    def loss_fn(params):
        ...
        if len(architecture["categorical_feature_sizes"]) > 0:
            ...
            for original_categorical, reconstructed_categorical in zip(
                ...
                #NOTE version with ideal distribution
                num_classes = reconstructed_categorical.shape[1]
                ideal_distribution = jnp.ones(reconstructed_categorical.shape) / num_classes
                balance_penalty = jnp.sum(jnp.square(reconstructed_categorical - ideal_distribution))
                balance_penalty_weight = search_params["simpson_weight"]
                
                #Two independent loss terms
                #balance_penalty: sum of square of differences between reconstructed and ideal distribution
                #feature_loss_simpson: (loss) simpson index of the reconstructed distribution (based on probabilities)
                #balance_penalty_weight: weight of the balance penalty (max weight defined by user)
                #balance_mask: mask to apply the balance penalty only to some columns (sensitive features)
                CE_categorical += loss_cat + loss_cat_noise
                balance_loss += balance_penalty*balance_penalty_weight*balance_mask[column_index]
                
                jax_log_debug("___FEATURE___[{x}]", x=num_classes)
                jax_log_debug("balance_mask: {x}", x=search_params["balance_mask"])
                jax_log_debug("reconstructed_categorical.shape: {x} - ideal_distribution.shape: {i}", x=reconstructed_categorical.shape, i=ideal_distribution.shape)
                # jax_log_debug("reconstructed_class_distribution: {x}", x=reconstructed_class_distribution)
                jax_log_debug("reconstructed_categorical[0]: {x}", x=reconstructed_categorical[0])
                jax_log_debug("ideal_distribution[0]: {x}", x=ideal_distribution[0])
                jax_log_debug("balance_penalty: {x}", x=balance_penalty)
                jax_log_debug("balance_mask[{c}] = {bm}", c=column_index, bm=balance_mask[column_index])
                jax_log_debug("[CE_cat] loss_cat:{lc}+loss_cat_noise:{lcn}={tot} [{cecat}]", lc=loss_cat, lcn=loss_cat_noise, tot=loss_cat+loss_cat_noise+balance_penalty*balance_penalty_weight, cecat=CE_categorical)
                jax_log_debug("[balance loss] {bp}*{bpw}*{msk}={tot} [{bl}]",bp=balance_penalty, bpw=balance_penalty_weight, msk=balance_mask[column_index], tot=balance_penalty*balance_penalty_weight*balance_mask[column_index], bl=balance_loss)
        ...
        loss = (jnp.mean(loss_ordinal + CE_categorical + balance_loss)
                + weight_penalty
                + search_params["alpha"] * loss_kld)
        jax_log_debug("ord: {o} _ cat: {c} _ bal: {b} = {oc}", o=jnp.mean(loss_ordinal), c=CE_categorical, b=balance_loss, oc=jnp.mean(loss_ordinal + CE_categorical + balance_loss))
