...
class Dataset(BaseModel):
    ...
    def set_bounds(cls, v, values):
        ...
        if "data" not in values:
            ...
            ordinal_cols = values["data"].select_dtypes(
            ...
            bounds = dict()
            for num in list(ordinal_cols.columns):
                bounds[num]: Dict = {
    ...
    def __init__(self,
        ...
        if target_column is not None and target_column not in data:
            logger.warning(f"Target column '{target_column}' is not a column in the dataset, target_column set as None (Unlabeled Dataset) ")
            target_column = None
        ...
        if target_column and drop_target_na_rows:
            ...
            if target_column_na_values > 0:
                logger.info(f"There are {target_column_na_values} rows containing na value in the target column, they will be dropped.")
                data.dropna(subset = [target_column], inplace=True)
                if len(data.index) == 0: raise ValueError("After removing the rows containing na value in the target column, the dataset is empty.")
    ...
    def ordinal_encoder(self, column: Union[str, Tuple]) -> None:
        """
    ...
    def _update_all_bounds(self) -> None:
        ...
        """
        ordinal_cols = self.subset_by_type(include=["number", "datetime", "timedelta"])
