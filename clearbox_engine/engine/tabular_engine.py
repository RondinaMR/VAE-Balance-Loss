    ...
    def fit(
        ...
        for _ in trange(epochs, desc="Engine fitting in progress", unit="epoch"):
            ...
            for batch in np.array_split(train_loader, splits, axis=0):
                ...
                # else:
                    # state, loss_value = train_step_simpsonized(self.hashed_architecture, state, batch, self.search_params)
                batch_num += 1
            ...
            losses.append(
                {'type': 'epoch', 'this_epoch': i, 'total_epochs': epochs, 'batch_num': batch_num, 'loss': epoch_loss})
