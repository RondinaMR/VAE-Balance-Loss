...
from clearbox_engine.VAE.tabular_vae_simpsonized import TabularVAESimpsonized, eval, train_step
from .engine import EngineInterface

import logging

# create logger
module_logger = logging.getLogger('engine.tabular_engine_simpsonized')


class TabularEngineSimpsonized(EngineInterface):
    model: TabularVAESimpsonized
    params: FrozenDict
    search_params: Dict
    architecture = Dict
    ...
    def __init__(
        ...
        self.model = TabularVAESimpsonized(
        ...
        if params:
            ...
            self.params = self.model.init(rng, x, y)["params"]
        self.search_params = train_params
        ...
        self.logger = logging.getLogger('engine.tabular_engine_simpsonized.TabularEngineSimpsonized')
        self.logger.info('creating an instance of TabularEngineSimpsonized')
    ...
    def fit(
        ...
        for _ in trange(epochs, desc="Engine fitting in progress", unit="epoch"):
            ...
            for batch in np.array_split(train_loader, splits, axis=0):
                #batch_in = np.tile(batch, (100,1))
                #print(batch.shape, batch_in.shape)
                # if self.simpson_weight == 0:
                #     state, loss_value = train_step(self.hashed_architecture, state, batch, self.search_params)
                # else:
            ...
            # print(f"\n[{i}/{epochs}] average epoch loss: {epoch_loss}\n")
            i += 1
